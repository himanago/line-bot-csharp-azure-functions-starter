...
### 7. Azure AI Foundryエージェントの設定（オプション）

Azure AI Foundryを使用してLINE Botに AI エージェントを統合することができます。

#### 7.1. Azure AI Foundryプロジェクトの作成

1. [Azure AI Foundry](https://ai.azure.com/) にアクセス
2. 新しいプロジェクトを作成
3. AI Agentを作成し、設定を完了

#### 7.2. 環境変数の設定

`local.settings.json` ファイル（ローカル開発）または Azure Functions の環境変数（本番環境）に以下を設定：
...
- ユーザーからのメッセージは AI Agent に送信されます
- AI Agent の処理中は「しばらくお待ちください...」メッセージが返されます
- AI Agent からの応答は自動的に LINE ユーザーに返信されます
- Durable Functions を使用して、複数のメッセージが同時に来ても適切に処理されます

### 8. オウム返しの動作確認と Bot の実装
​
